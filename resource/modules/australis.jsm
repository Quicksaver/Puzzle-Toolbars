...
this.trackSpecialWidgets = {
	onWidgetAdded: function(aId, aCurrentArea, aCurrentPosition) {
		if(aId.startsWith(objName+'-special-')) {
			...
			CustomizableUI.removeWidgetFromArea(aId, aCurrentArea);
...
moduleAid.LOADMODULE = function() {
	...
	for(var i=0; i<specialWidgets.length; i++) {
		var placement = CustomizableUI.getPlacementOfWidget(objName+'-special-'+specialWidgets[i]);
		if(placement) { CustomizableUI.removeWidgetFromArea(objName+'-special-'+specialWidgets[i], placement.area); }
	}
